...
* __COOKIEFILE_ON_NO_FORMATS_ONLY__: If `true` and `cookiefile` is provided via `YTDL_OPTIONS`/`YTDL_OPTIONS_FILE`, MeTube will try the download without cookies first and only retry with cookies when it sees "No video formats found!". Defaults to `false`.
